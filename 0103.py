    ...
    while not game_over:
        try:
            ...
            if "Game Over" in message:
                game_over_screen(message)  # 显示游戏结束界面
                game_over = True
                break
...
def parse_game_over_message(message):
    """
    解析传入的游戏结束消息字符串，提取玩家昵称、分数和头像路径。
    返回一个列表，包含每个玩家的 (nickname, score, avatar_image_path)
    """
    lines = message.split("\n")  # 按行分割
    players_info = []
    ...
    for line in lines:
        if line.startswith("Scores:"):
            continue  # 跳过 "Scores:" 这一行
        if line.strip():  # 如果有内容
            parts = line.split(":")  # 分割每行
            if len(parts) == 3:
                nickname = parts[0].strip()
                score = int(parts[1].strip())
                avatar_image_path = parts[2].strip()
                players_info.append((nickname, score, avatar_image_path))

    return players_info

def game_over_screen(message):
    # 解析游戏结束信息
    players_info = parse_game_over_message(message)

    while True:
        screen.fill((255, 255, 255))  # 填充白色背景
        clock = pygame.time.Clock()
        # 设置游戏结束背景
        end_background = pygame.image.load('end2_background.jpg')  # 加载背景图片
        ...
        # 显示游戏结束标题
        title = font.render("遊戲結束", True, (0, 0, 0))  # 黑色文字
        screen.blit(title, (screen_width // 2 - title.get_width() // 2, 50))

        # 显示头像、昵称和分数，分成三行显示
        y_offset = 150  # 开始显示玩家信息的Y坐标

        # 显示头像1，头像2 (左边头像1，右边头像2)
        y_offset = 150
        for i, (_, _, avatar_image_path) in enumerate(players_info):
            if avatar_image_path:
                try:
                    avatar = pygame.image.load(avatar_image_path)
                    avatar = pygame.transform.scale(avatar, (250, 250))  # 调整头像大小
                    if i == 0:  # 玩家1头像在左
                        x_offset = screen_width // 4 - 100
                    else:  # 玩家2头像在右
                        x_offset = 3 * screen_width // 4 - 100
                    screen.blit(avatar, (x_offset, y_offset))  # 绘制头像
                except pygame.error:
                    print(f"Error loading image {avatar_image_path}")

        y_offset += 250  # 头像行之后间隔一些空间

        # 昵称1，昵称2 (左边昵称1，右边昵称2)
        for i, (nickname, _, _) in enumerate(players_info):
            nickname_text = font.render(f"暱稱: {nickname}", True, (0, 0, 0))  # 黑色文字
            if i == 0:  # 玩家1昵称在左
                x_offset = screen_width // 4 - nickname_text.get_width() // 2
            else:  # 玩家2昵称在右
                x_offset = 3 * screen_width // 4 - nickname_text.get_width() // 2
            screen.blit(nickname_text, (x_offset, y_offset))  # 绘制昵称

        y_offset += 120  # 昵称行之后间隔一些空间

        # 分数1，分数2 (左边分数1，右边分数2)
        for i, (_, score, _) in enumerate(players_info):
            score_text = font.render(f"分數: {score}", True, (0, 0, 0))  # 黑色文字
            if i == 0:  # 玩家1分数在左
                x_offset = screen_width // 4 - score_text.get_width() // 2
            else:  # 玩家2分数在右
                x_offset = 3 * screen_width // 4 - score_text.get_width() // 2
            screen.blit(score_text, (x_offset, y_offset))  # 绘制分数
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                if back_to_main_button.is_clicked(event.pos):  # 按下返回主页面按钮
                    return 'back_to_main'  # 返回主页面
